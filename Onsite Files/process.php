<?php
// To be used in Onsite contests with one time password.
// Generates new password on each Login.
...
// ------------------ LOGIN ------------------- //
    if (isset($_POST['login'])) {
		...
		// ----- One Time Password ---- //
		if($res['teamname'] != "ACM") {
                	$newPassword = addslashes(generatePassword());
                	$query = "update teams set pass = '$newPassword' where teamname = '$_POST[teamname]' and pass = '$_POST[password]'";
                	$res1 = DB::query($query);
		}
                // ------- //
                $save = $_SESSION;
